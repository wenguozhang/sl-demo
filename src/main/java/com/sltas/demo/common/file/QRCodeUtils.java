...
public class QRCodeUtils {
	    ...
	    public static String encode(String content, String imgPath, String destPath,
	                              boolean needCompress,String picName){
	        try {
	        	destPath = destPath + "userInfoQR";
				...
				if(StringUtils.isBlank(picName)){
					file = (UUID.randomUUID().toString()).replaceAll("-", "")+".jpg";
				}else{
					file = picName + new Date().getTime()+".jpg";
				}
				
				//String file = new Random().nextInt(99999999)+".jpg";1
				ImageIO.write(image, FORMAT_NAME, new File(destPath+"/"+file));
				return "userInfoQR"+"/"+file;
			}catch (Exception e) {
				logger.error("内嵌LOGO---出现系统异常---e[{}]",e);
				return "";
			}
	    }
